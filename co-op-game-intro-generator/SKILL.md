...
local-fallback: |
  Claude Code / generic agent harness: 跳过 canvas 图像生成。
  只输出：视觉风格候选、玩家信息表、固定 UI 框架描述、确认图 H3 文生图 prompt（含模板字段填充）、
  最终 H3 视频提示词（含模板字段填充）。
  用户拿到 prompt 后自行到 H3 平台生成确认图与视频。
  references/h3-confirmation-image-template.md 与 references/h3-video-prompt-template.md 仍作为模板来源。
---
