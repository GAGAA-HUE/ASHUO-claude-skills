...
| 技能名 | 类型 | 功能描述 |
|--------|------|----------|
| `art-asset-designer` | AI 绘画 | 美术资产设计师：从剧本/故事拆解人物与场景资产，生成可直接用于 Midjourney、Flux、SD 等 AI 图像工具的分类提示词 |
...
**适用场景**：影视/动画/游戏美术资产生成，从剧本/故事/分镜拆解人物与场景，并输出可直接用于 AI 图像生成工具的高质量提示词

**核心能力**：
- **剧本解析**：自动提取所有人物（含次要人物）与独立场景，整理完整资产清单
- **风格确认**（必须步骤）：在出图前向用户询问视觉风格、影像质感、参考方向，建立统一「风格档案」
- **人物提示词**：每个角色生成两套——半身正面特写（3:4 竖版）+ 三视图角色设计表（16:9 横版）
- **外观档案**：完整构建角色视觉信息（肤色/五官/发型/服装/气质），推断补全内容用斜体标注
- **场景提示词**：纯环境出图，禁止出现任何人物，16:9 横版，强调氛围与材质
- **风格一致性**：人物与场景提示词统一套用风格档案，保证整部作品视觉风格统一
- 支持 Midjourney、Stable Diffusion、DALL-E、Flux 等主流 AI 图像工具

**工作流程**：
1. STEP 1 — 剧本解析，输出资产清单（人物 + 场景）
2. STEP 1.5 — 风格确认（**禁止跳过**，必须向用户询问三项风格问题）
3. STEP 2 — 生成人物提示词（套装A 半身特写 + 套装B 三视图）
4. STEP 3 — 生成场景提示词（纯环境，无人物）

**触发方式**：
```
@art-asset-designer 帮我拆解这个剧本的美术资产
@art-asset-designer 我需要为这个故事生成角色设计图和场景图
@art-asset-designer 帮我出这个脚本的人物和场景提示词
```
