...
local-fallback: |
  Claude Code / generic agent harness: 跳过 canvas 写入、choice cards 与 hub_generate_*。
  按官方 18 步顺序在对话中产出：
  项目简报、故事大纲、角色卡、场景卡、六列分镜表（per-second + 音频 + 空间锚链）、
  文字 storyboard（每个 shot 一节；heavy-iteration shot 抽为独立节）、视频模型选择建议、
  单 shot H3 提示词、组装方案、BGM 匹配、最终 review。
  不模拟生成或剪辑；长文档写入 references/outputs/。
---
