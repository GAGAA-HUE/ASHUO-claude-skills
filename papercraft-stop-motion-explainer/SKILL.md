...
local-fallback: |
  Claude Code / generic agent harness: 跳过 canvas 写入。
  只输出：学习目标、视觉隐喻、风格规则、角色与场景设计、资产清单、preview 概念、
  图像/图像序列/视频 H3 提示词、分镜、镜头与转场、声音方向、负向 prompt、review checklist。
  默认 MiniMax-H3；用户明确指定其他模型或 MiniMax-H3 不可用时切换。
  长文档写入 references/outputs/。
---
