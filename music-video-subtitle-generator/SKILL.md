...
local-fallback: |
  Claude Code / generic agent harness: 跳过 canvas 写入与 Hub agent 委派。
  只输出：MV 概念、beat grid、跨镜头锚链、各 shot 的 H3 提示词、歌词文字计划、质检清单。
  长文档写入 references/outputs/，聊天里只给摘要。
  用户歌词逐字不改；不补写、不翻译、不释义。
