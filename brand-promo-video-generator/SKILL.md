...
local-fallback: |
  Claude Code / generic agent harness: 跳过 canvas 与 hub_* 工具调用。
  只输出：资产真伪 manifest、叙事方向、节拍/分镜表、各镜头 H3 提示词、文案逐字台本、质检清单。
  长文档写入 references/outputs/，聊天里只给摘要与下一步。
  不得自行编造 logo / wordmark / 包装 / 官方 UI / 真实人物。
allowed-tools:
- webfetch
- hub_image_search
- hub_analyse_media
