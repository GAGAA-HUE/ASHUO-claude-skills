...
**单条提示词字数边界**：
每个可直接复制给 Seedance 2.0 的成品提示词，字数控制在 800-2000 字之间。
- 下限 800：确保内容丰度足够支撑生成稳定性
- 上限 2000：匹配 Seedance 2.0 的上下文窗口与解析效率

**剧本/分镜脚本的拆分规则**：

按以下优先级识别用户内容中的自然分段：
1. 明确的场次/场景编号转换（如"场景1"→"场景2"）
2. 时间/空间跳跃（如"三天后""切换到室外"）
3. 视角或主体切换（如"切到反派视角""特写手部"）
4. 长镜头内部按秒或按阶段的推进（仅在单条内容超出2000字时考虑）

**独立性保障**：每条提示词必须包含该场的核心角色状态、空间信息、核心动作或事件、结尾状态。即使与前一场是同一角色、同一地点，也必须简要重述关键信息。禁止假设模型会"记住"前一条提示词的内容。

**字数超限时的处理**：
单条超出 2000 字时，优先在该条内部压缩非核心细节，而非改变用户的结构划分。
...
3. 判断输出模式：长镜头 / 多分镜 / 蒙太奇
4. 如为动作戏，再判断：普通动作 / 变身或异变或强烈前摇动作
5. 提炼最影响生成结果的信息：人物、道具、场景、材质、动作、镜头、节奏、特效、声音、结尾状态
6. 仅在必要时补充更细镜头控制，避免无效信息膨胀
...
【字数层】
□ 单条提示词是否在 800-2000 字范围内？
□ 若超出 2000，是否优先压缩非核心细节而非改变结构？
